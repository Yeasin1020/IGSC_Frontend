...
const EnrollmentsManagement = () => {
  ...
  const updateStatus = async (id: string, nextStatus: string) => {
    try {
      await enrollmentsApi.update(id, { status: nextStatus });
      await load();
    } catch (err) {
  ...
  return (
    <div className="space-y-5">
      <AdminPageHeader
        title="Enrollments"
        subtitle="Review and update course registrations"
      />
            ...
            <AdminTableBody>
              {items.map((item) => (
                <tr key={item._id}>
                  ...
                  <AdminTableCell>{item.courseSnapshot.title}</AdminTableCell>
                  <AdminTableCell>
                    <StatusBadge status={item.status} />
                  </AdminTableCell>
                  <AdminTableCell>
